...
export default function CreateRps({ curriculums, courses, defaultAcademicYear }: { curriculums: Curriculum[]; courses: Course[]; defaultAcademicYear: string }) {
    ...
    return <>
        ...
        <div className="p-4 md:p-6">
            ...
            <form onSubmit={(e) => { e.preventDefault(); if (!form.processing) form.post('/rps'); }} className="mt-6 grid gap-5 xl:grid-cols-[1.05fr_.95fr]">
                ...
                <section className="sim-surface rounded-2xl p-5">
                    ...
                    {selected ? <>
                        ...
                        <button type="submit" disabled={selected.generator_readiness==='needs_admin_review' || form.processing} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-teal-700 px-5 py-3 text-sm font-bold text-white disabled:opacity-45"><BookOpenCheck className="size-4" />{form.processing ? 'Membuat Draft…' : 'Buat Draft RPS'}</button>
