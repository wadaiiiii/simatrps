    ...
    private function compactForType(array $full, ?string $type): array
    {
        ...
        return match ($type) {
            ...
            'reference_plan' => $base + [
                ...
                'existing_materials' => $full['materials'],
                'weekly_plan' => collect($full['weekly_plan'])
                    ->map(fn (array $week): array => [
                        'week_number' => $week['week_number'],
                        'sub_cpmk_code' => $week['sub_cpmk_code'],
                        'material' => $this->clip($week['material'] ?? null, 200),
                    ])
                    ->all(),
                'existing_references' => [
                    'main' => $full['document']['reference_text'] ?? [],
                    'supporting' => $full['document']['supporting_reference_text'] ?? [],
                ],
            ],
            ...
            'weekly_plan' => $base + [
                ...
                'bibliography' => $this->bibliographyEntries(
                    ...
                    ) ?: (string) ($full['master_syllabus']['references'] ?? '')
                ),
            ],
