...
class RpsAiController extends Controller
{
    ...
    public function apply(Request $request, string $rps, string $suggestion): RedirectResponse
    {
        ...
        if (in_array($row->suggestion_type, ['cpmk_review', 'sub_cpmk'], true) && $selectedIndices === []) {
            throw ValidationException::withMessages([
                'ai' => 'Pilih minimal satu rekomendasi yang akan diterapkan.',
            ]);
        }

        $result = DB::transaction(function () use ($row, $payload, $selectedIndices, $record, $version, $request): array {
            ...
            if (($result['changed'] ?? 0) < 1 && in_array($row->suggestion_type, ['cpmk_review', 'sub_cpmk'], true)) {
                throw ValidationException::withMessages([
    ...
    private function applyAssessmentPlan(array $payload, object $version, int $userId): array
    {
        ...
        return [
            'changed' => count($assessments) + count($payload['tasks'] ?? []),
            'message' => 'Rencana asesmen dan RTM AI diterapkan.',
        ];
    }
