...
class ObeWorkspaceController extends Controller
{
    ...
    public function storeMaterial(Request $request, string $rps): RedirectResponse
    {
        ...
        $requestedSubIds = collect($data['sub_cpmk_ids'] ?? [])
            ->when(
                filled($data['rps_sub_cpmk_id'] ?? null),
                fn ($items) => $items->push($data['rps_sub_cpmk_id'])
            )
            ->filter()
            ->unique()
            ->values();

        $validSubIds = $requestedSubIds->isEmpty()
            ? collect()
            : DB::table('rps_sub_cpmks')
                ->where('rps_version_id', $version->id)
        ...
        abort_unless($validSubIds->count() === $requestedSubIds->count(), 422);

        DB::table('rps_materials')->insert([
            'id' => $materialId,
            'rps_version_id' => $version->id,
            'rps_sub_cpmk_id' => $validSubIds->first(),
            'title' => $data['title'],
        ...
        $this->syncMaterialSubCpmks($materialId, $validSubIds->all());
    ...
    public function updateMaterial(
        ...
        $this->syncMaterialSubCpmks($material, $validSubIds->all());

        return back()->with('success', 'Bahan kajian dan keterkaitannya berhasil diperbarui.');
    }
