...
class ObeWorkspaceService
{
    public function progress(string $versionId): array
    {
        ...
        $materialCoveredSubCount = 0;

        if ($subCpmkIds->isNotEmpty()) {
            if (Schema::hasTable('rps_material_subcpmks')) {
                $materialCoveredSubCount = DB::table('rps_material_subcpmks')
                    ->whereIn('rps_sub_cpmk_id', $subCpmkIds)
                    ->distinct()
                    ->count('rps_sub_cpmk_id');
            } else {
                $materialCoveredSubCount = DB::table('rps_materials')
                    ->where('rps_version_id', $versionId)
        ...
        $checks = [
            ...
            [
                ...
                'done' => $materials > 0
                    && $subCpmks->isNotEmpty()
                    && $materialCoveredSubCount === $subCpmks->count(),
                'message' => "{$materials} bahan kajian tersedia; "
                    ."{$materialCoveredSubCount}/{$subCpmks->count()} Sub-CPMK didukung bahan kajian.",
            ],
            [
                'key' => 'weeks',
                'label' => '16 Pertemuan',
