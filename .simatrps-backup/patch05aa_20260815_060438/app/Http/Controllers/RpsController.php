    ...
    public function show(
        ...
        $materials = DB::table('rps_materials')
            ...
            ->map(function ($material): object {
                $linkedIds = Schema::hasTable('rps_material_subcpmks')
                    ? DB::table('rps_material_subcpmks')
                        ->where('rps_material_id', $material->id)
                        ->pluck('rps_sub_cpmk_id')
                        ->all()
                    : [];

                if (empty($linkedIds) && filled($material->rps_sub_cpmk_id ?? null)) {
                    $linkedIds = [$material->rps_sub_cpmk_id];
                }

                $material->sub_cpmk_ids = array_values(array_unique($linkedIds));
