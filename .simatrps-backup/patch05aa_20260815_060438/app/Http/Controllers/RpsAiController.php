...
class RpsAiController extends Controller
{
    ...
    private function applyMaterialPlan(
        ...
        foreach ($selectedIndices as $index) {
            ...
            $subCodes = collect($item['sub_cpmk_codes'] ?? [])
                ->when(
                    filled($item['sub_cpmk_code'] ?? null),
                    fn ($items) => $items->push($item['sub_cpmk_code'])
                )
            ...
            if ($existing) {
                ...
                if (Schema::hasTable('rps_material_subcpmks') && $subIds->isNotEmpty()) {
                    DB::table('rps_material_subcpmks')
                        ->where('rps_material_id', $existing->id)
                        ->delete();

                    foreach ($subIds as $linkedSubId) {
                        DB::table('rps_material_subcpmks')->insert([
                            'rps_material_id' => $existing->id,
                            'rps_sub_cpmk_id' => $linkedSubId,
                            'created_at' => now(),
                            'updated_at' => now(),
                        ]);
                    }
