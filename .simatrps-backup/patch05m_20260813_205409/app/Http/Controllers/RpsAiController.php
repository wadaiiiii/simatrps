...
class RpsAiController extends Controller
{
    ...
    private function defaultTimeEstimate(int $credits): string
    {
        ...
        return "Tatap muka: {$credits}×50 menit; "
            ."Tugas terstruktur: {$credits}×60 menit; "
            ."Belajar mandiri: {$credits}×60 menit";
    }
    ...
    private function applyAssessmentPlanSelective(
        ...
        if ($changedAssessments > 0 && $totalWeight > 100.0) {
            throw ValidationException::withMessages([
                'ai' => "Penerapan asesmen AI akan membuat total bobot {$totalWeight}%. Transaksi dibatalkan agar bobot tidak melebihi 100%.",
            ]);
        }

        if ($changedAssessments > 0 && abs($totalWeight - 100.0) >= 0.01) {
            $message .= " Total bobot asesmen saat ini {$totalWeight}%; lengkapi hingga tepat 100% sebelum finalisasi.";
        }
