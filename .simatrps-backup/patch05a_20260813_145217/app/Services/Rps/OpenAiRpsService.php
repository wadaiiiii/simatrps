...
class OpenAiRpsService
{
    public function isConfigured(): bool
    {
        return filled(config('simatrps-ai.api_key'));
    }

    public function model(): string
    {
        return (string) config('simatrps-ai.model', 'gpt-5.6-terra');
    }
    ...
    public function testConnection(): array
    {
        ...
        $response = $this->client()
            ->get(config('simatrps-ai.base_url').'/models/'.$this->model());
    ...
    public function generate(string $type, array $context, ?string $instruction = null): array
    {
        ...
        $response = $this->client()->post(
            config('simatrps-ai.base_url').'/responses',
            [
                'model' => $this->model(),
                'input' => $input,
                'store' => false,
                'max_output_tokens' => (int) config('simatrps-ai.max_output_tokens', 12000),
                'text' => [
                    'format' => [
                        'type' => 'json_schema',
                        'name' => 'simatrps_'.$type,
        ...
        if (! $text) {
            $refusal = $this->extractRefusal($json);

            throw ValidationException::withMessages([
                'ai' => $refusal
                    ? 'AI tidak dapat membuat rekomendasi: '.$refusal
                    : 'OpenAI tidak mengembalikan output teks yang dapat diproses.',
            ]);
        }
    ...
    {
        return Http::withToken((string) config('simatrps-ai.api_key'))
            ->acceptJson()
            ->asJson()
            ->timeout((int) config('simatrps-ai.timeout', 120))
            ->retry(1, 600, throw: false);
    }
    ...
    private function assertConfigured(): void
    {
        if (! $this->isConfigured()) {
            throw ValidationException::withMessages([
                'ai' => 'OpenAI API belum dikonfigurasi. Jalankan: herd php artisan simatrps:ai-config',
            ]);
        }
    }
    ...
    private function throwApiError(int $status, mixed $body): never
    {
        ...
        $friendly = match ($status) {
            401 => 'API key OpenAI tidak valid atau tidak memiliki akses.',
            403 => 'Akses API OpenAI ditolak untuk project/model ini.',
            404 => 'Model OpenAI yang dipilih tidak ditemukan atau tidak tersedia.',
            429 => 'Batas penggunaan/kuota API OpenAI tercapai. Periksa billing dan usage API.',
            default => 'Permintaan ke OpenAI gagal (HTTP '.$status.').',
        };
        ...
        foreach (($response['output'] ?? []) as $output) {
            foreach (($output['content'] ?? []) as $content) {
                if (($content['type'] ?? null) === 'refusal') {
                    return $content['refusal'] ?? 'Permintaan ditolak.';
                }
            }
        }
