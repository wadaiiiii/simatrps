...
abstract class OpenAiCompatibleBackupRpsService
{
    ...
    {
        return config('simatrps-ai.'.$this->providerKey().'.'.$key, $default);
    }

    public function isConfigured(): bool
    {
        return filled($this->providerConfig('api_key'));
    }

    public function model(): string
    {
        return (string) $this->providerConfig('model', $this->defaultModel());
    }
    ...
    public function testConnection(): array
    {
        ...
        $response = $this->client()->post(
            $this->providerConfig('base_url').'/chat/completions',
            [
                'model' => $this->model(),
                'messages' => [
    ...
    public function generate(string $type, array $context, ?string $instruction = null): array
    {
        ...
        $response = $this->postWithRateLimitRetry(
            ...
            [
                ...
                'temperature' => 0.2,
                'max_tokens' => $this->maxCompletionTokens($type),
                'response_format' => [
                    'type' => 'json_object',
                ],
            ]
        ...
        if (! is_string($text) || trim($text) === '') {
            throw ValidationException::withMessages([
                'ai' => $this->providerLabel().' tidak mengembalikan output JSON yang dapat diproses.',
            ]);
        }
    ...
    public function generateWeeklyBatch(array $context, array $targetWeeks, ?string $instruction = null): array
    {
        ...
        $response = $this->postWithRateLimitRetry(
            ...
            [
                ...
                'temperature' => 0.15,
                'max_tokens' => min(850, 420 + (count($targetWeeks) * 330)),
                'response_format' => [
                    'type' => 'json_object',
                ],
            ]
        ...
        if (! is_string($text) || trim($text) === '') {
            throw ValidationException::withMessages(['ai' => $this->providerLabel().' tidak mengembalikan batch rencana mingguan.']);
        }

        try {
            $payload = json_decode($text, true, 512, JSON_THROW_ON_ERROR);
        } catch (\JsonException $e) {
            throw ValidationException::withMessages(['ai' => 'Output batch '.$this->providerLabel().' bukan JSON valid: '.$e->getMessage()]);
        }

        $this->validateWeeklyBatch($payload, $targetWeeks, $this->providerLabel());
    ...
    private function postWithRateLimitRetry(string $url, array $payload)
    {
        ...
        for ($attempt = 1; $attempt <= 2; $attempt++) {
            ...
            // Tidak semua model gratis/router mendukung response_format.
            // Prompt tetap memaksa JSON, jadi ulang sekali tanpa parameter
            // tersebut bila provider menolaknya.
            if (
                $response->status() === 400
                ...
                $response = $this->client()->post($url, $fallbackPayload);
                $last = $response;
            }
    ...
    {
        $client = Http::withToken((string) $this->providerConfig('api_key'))
            ->acceptJson()
            ->asJson()
            ->timeout(min((int) $this->providerConfig('timeout', 22), 22));

        if ($this->providerKey() === 'openrouter') {
            $client = $client->withHeaders([
                'HTTP-Referer' => config('app.url', 'http://simatrps.test'),
                'X-OpenRouter-Title' => 'SiMatRPS',
            ]);
        }
    ...
    private function assertConfigured(): void
    {
        if (! $this->isConfigured()) {
            throw ValidationException::withMessages([
                'ai' => $this->providerLabel().' API belum dikonfigurasi.',
            ]);
        }
    }
    ...
    private function throwApiError(int $status, mixed $body): never
    {
        ...
        $friendly = match ($status) {
            400 => 'Permintaan ke '.$this->providerLabel().' tidak dapat diproses.',
            ...
            401 => 'API key '.$this->providerLabel().' tidak valid.',
            403 => 'Akses '.$this->providerLabel().' ditolak untuk project/model ini.',
            404 => 'Model '.$this->providerLabel().' yang dipilih tidak ditemukan atau tidak tersedia.',
            429 => 'Batas penggunaan '.$this->providerLabel().' sedang tercapai. SiMatRPS sudah mencoba menunggu dan mengulang otomatis.',
            default => 'Permintaan ke '.$this->providerLabel().' gagal (HTTP '.$status.').',
        };
...
Tugas: telaah Bahan Kajian RPS sebagai GAP ANALYSIS antara `existing_materials`, `syllabus_items`, CPMK, dan seluruh Sub-CPMK aktif. Gunakan HANYA topik yang dapat diturunkan dari silabus dan capaian yang tersedia; jangan mengarang topik di luar konteks. Pertahankan bahan kajian kurikulum sebagai sumber utama. Gunakan `link` bila bahan kajian yang sudah ada hanya perlu dikaitkan ke satu atau beberapa Sub-CPMK; `adapt` bila judul perlu dirapikan tanpa mengubah substansi; dan `add` hanya jika Sub-CPMK penting belum didukung materi yang tersedia tetapi topiknya masih dapat diturunkan dari silabus/konteks RPS. Hindari duplikasi. Satu bahan kajian BOLEH mendukung beberapa Sub-CPMK. Usahakan seluruh Sub-CPMK mempunyai minimal satu bahan kajian yang relevan.
PROMPT,
...
PROMPT,
            'weekly_plan' => <<<'PROMPT'
Tugas: susun rencana pembelajaran hanya untuk minggu target yang diberikan. Gunakan `target_sub_cpmk` yang sudah ditetapkan sistem; JANGAN memilih atau mengganti Sub-CPMK lain. Materi harus berasal dari `materials` atau `syllabus_items`. Pilih bentuk dan metode pembelajaran yang sesuai dengan Bloom. Estimasi waktu harus mengikuti `time_standard` dari sistem. Buat penugasan, aktivitas, indikator, kriteria, teknik asesmen, dan referensi secara sangat ringkas. Jangan mengarang pustaka. Untuk field `references`, pilih kode pustaka dari `bibliography` yang benar-benar relevan dengan materi minggu tersebut. Bila konteks menyediakan beberapa pustaka yang relevan, gunakan 2-4 kode seperti `[1], [3], [5]`; jangan otomatis memakai `[1]` untuk semua minggu dan variasikan sumber sesuai topik.
PROMPT,
            ...
            'properties' => [
                ...
                'items' => [
                    ...
                    'items' => [
                        'type' => 'object',
                        'properties' => [
                            'action' => ['type' => 'string', 'enum' => ['link', 'adapt', 'add']],
                            'target_title' => ['type' => ['string', 'null']],
                            'title' => ['type' => 'string'],
                            'sub_cpmk_code' => ['type' => ['string', 'null']],
                            'sub_cpmk_codes' => [
                                'type' => 'array',
                                'items' => ['type' => 'string'],
                                'maxItems' => 14,
                        ...
                        'required' => ['action','target_title','title','sub_cpmk_code','sub_cpmk_codes','rationale'],
                        'additionalProperties' => false,
                    ],
                ],
            ],
