...
class GroqRpsService
{
    ...
    {
        return (string) config('simatrps-ai.groq.model', 'openai/gpt-oss-20b');
    }
    ...
    private function client(): PendingRequest
    {
        return Http::withToken((string) config('simatrps-ai.groq.api_key'))
            ->acceptJson()
            ->asJson()
            ->timeout(min((int) config('simatrps-ai.groq.timeout', 22), 22));
    }
    ...
    private function throwApiError(int $status, mixed $body): never
    {
        ...
        $friendly = match ($status) {
            ...
            429 => 'Batas token Groq Free sedang penuh. SiMatRPS sudah mencoba menunggu dan mengulang otomatis; coba lagi beberapa saat jika masih gagal.',
